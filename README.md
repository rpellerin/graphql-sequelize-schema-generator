...
var schema = GraphQLSchema(generateSchema(models)) // Generates the schema
// OR
...
const {generateModelTypes, generateSchema} = require('graphql-sequelize-schema-generator')
...
app.use(
  '/graphql',
  graphqlHTTP({
    schema: GraphQLSchema(generateSchema(models)),
    graphiql: true
  })
)
